...
function orderType(array, target) {
    const length = array.length
    let start = 0, end = length-1
    ...
    while (start<=end) {
        const mid = Math.floor((start+end)/2)
        if (array[mid] === target) {
            return [mid, isAscending ? 'Ascending' : 'Dscending']
        }
        if (isAscending) {
            if (array[mid] < target) {
                start = mid+1
...
console.log(orderType([1, 3, 5, 7, 9], 7));          // 3 (ascending)
console.log(orderType([22, 21, 16, 10, 9, 8, 7], 9)); // 4 (descending)
console.log(orderType([10, 9, 8, 7], 5));             // -1
