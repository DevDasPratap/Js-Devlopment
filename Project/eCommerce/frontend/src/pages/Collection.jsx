...
export const Collection = () => {
  ...
  const [subcCtegory, setSubCategory] = useState([])
  ...
  const toggleSubCategory = (e)=>{
    if(subcCtegory.includes(e.target.value)){
  ...
  const applyFilter = ()=>{
    ...
    if (subcCtegory.length > 0) {
      productCopy = productCopy.filter(item => subcCtegory.includes(item.subCategory));
    }
    // search filter
  ...
  useEffect(() => {
    ...
    // console.log('subcCtegory', subcCtegory)
    applyFilter()
  }, [category, subcCtegory, search, showSearch])
    ...
    <div className='flex flex-col sm:flex-row gap-1 sm:gap-10 pt-10 border-t'>
        ...
        <div className="min-w-60">
          ...
          <div className={`border border-gray-300 pl-5 py-3 my-5 ${showFilter ? '' : 'hidden'} sm:block`}>
            ...
            <div className='flex flex-col gap-2 text-sm font-light text-gray-700'>
              ...
              <p className='flex gap-2'>
                <input className='w-3' type='checkbox' value={'Buttomwear'} onChange={toggleSubCategory} />Buttomwear
              </p>
              <p className='flex gap-2'>
      ...
      <div className='flex-1'>
        <div className='flex justify-between text-base sm:text-2xl mb-4'>
          ...
          {/* Product sort */}
          <select className='boder border-gray-300 text-sm px-2' onChange={(e)=>setSortType(e.target.value)}>
